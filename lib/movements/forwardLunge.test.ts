...
// Bottom of a clean forward lunge on the left (camera-near = front) leg: front
...
describe("scoreMovement (forward_lunge spec)", () => {
  it("gives a deep, upright forward lunge full marks", () => {
    const result = scoreMovement(forwardLunge, [deepUprightFrame])
    expect(result.movementId).toBe("forward_lunge")
  ...
  it("scores a shallow front knee low on depth", () => {
    const result = scoreMovement(forwardLunge, [shallowFrame])
    ...
    expect(result.total).toBeLessThan(
      scoreMovement(forwardLunge, [deepUprightFrame]).total,
    )
  })

  it("scores a big forward lean low on torso while depth stays high", () => {
    const result = scoreMovement(forwardLunge, [bigLeanFrame])
    expect(dim(result, "depth").score).toBe(100)
  ...
  it("flags low confidence when a scoring joint is weak", () => {
    ...
    expect(scoreMovement(forwardLunge, [weak]).lowConfidence).toBe(true)
  })

  it("selects the deepest front-knee frame across a sequence", () => {
    const result = scoreMovement(forwardLunge, [
      shallowFrame,
      deepUprightFrame,
      shallowFrame,
    ])
  ...
  it("throws on an empty sequence", () => {
    expect(() => scoreMovement(forwardLunge, [])).toThrow()
  })
})
